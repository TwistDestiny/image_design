...
CimagedesignApp::CimagedesignApp() noexcept
{
	...
	SetAppID(_T("imagedesign.AppID.NoVersion"));

	// TODO:  在此处添加构造代码，
	// 将所有重要的初始化放置在 InitInstance 中
}

// 唯一的 CimagedesignApp 对象

CimagedesignApp theApp;
...
BOOL CimagedesignApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CimagedesignDoc),
		RUNTIME_CLASS(CMainFrame),       // 主 SDI 框架窗口
		RUNTIME_CLASS(CimagedesignView));
	if (!pDocTemplate)
		return FALSE;
...
// 用于运行对话框的应用程序命令
void CimagedesignApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
